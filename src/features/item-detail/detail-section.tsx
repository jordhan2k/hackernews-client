...
import { Poll, PollSkeleton } from "./components/poll";
...
function DetailSection({ item }: DetailSectionProps) {
  const { time, text, by, descendants, score, title, type, id, parts } = item;
  return (
    <section className="flex flex-col gap-9 md:gap-12">
      ...
      {text ? (
        <div className="text-base font-normal text-secondary-foreground md:text-lg flex flex-col gap-6">
          {parse(text)}
        </div>
      ) : null}
